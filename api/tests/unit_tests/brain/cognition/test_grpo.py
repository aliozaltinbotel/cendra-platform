...
from core.brain.cognition.grpo import (
    GRPOMetrics,
    GRPOTrainer,
...
def _build_table(
    rewards: Sequence[tuple[Mapping[str, float], MemoryOpKind, float]],
) -> dict[tuple[frozenset[tuple[str, float]], MemoryOpKind], float]:
    return {(frozenset(features.items()), kind): reward for features, kind, reward in rewards}
...
def test_grpo_l2_shrinks_weights() -> None:
    ...
    with_l2_w = abs(with_l2.weights.weights[MemoryOpKind.ADD]["x"])
