...
from core.brain.cognition.trainer import (
    SGDTrainer,
    TrainingMetrics,
    TrainingSample,
    iter_samples,
...
def test_train_learns_separable_pattern() -> None:
    ...
    samples = iter_samples([({"x": 1.0}, MemoryOpKind.ADD, 1.0)] * 10 + [({"x": -1.0}, MemoryOpKind.DELETE, 1.0)] * 10)
...
def test_l2_pulls_weights_toward_zero() -> None:
    ...
    no_l2 = abs(policy_no_l2.weights.weights[MemoryOpKind.ADD]["x"])
    with_l2 = abs(policy_with_l2.weights.weights[MemoryOpKind.ADD]["x"])
