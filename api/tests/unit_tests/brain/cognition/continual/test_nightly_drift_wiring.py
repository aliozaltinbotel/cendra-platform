...
def test_skips_when_case_store_missing() -> None:
    ...
    result = consolidator._step9_detect_foundation_drift()
    assert result == {
        "skipped": True,
...
def test_skips_when_update_store_missing() -> None:
...
def test_threshold_crossed_emits_candidate() -> None:
    ...
    stats = consolidator._step9_detect_foundation_drift()
    assert stats["cases_scanned"] == 3
    assert stats["candidates_emitted"] == 1
    assert stats["errors"] == 0
    pending = update_store.list_pending()
...
def test_below_threshold_emits_nothing() -> None:
    ...
    assert update_store.list_pending() == ()


def test_multi_scenario_emits_separate_candidates() -> None:
    ...
    scenario_ids = {candidate.foundation_scenario_id for candidate in update_store.list_pending()}
...
class _BadStore(InMemoryFoundationUpdateStore):
    ...
    def upsert(
        ...
        super().upsert(candidate)


def test_upsert_failure_does_not_break_the_step() -> None:
    ...
    assert stats["candidates_emitted"] == 1
    pending = bad_store.list_pending()
    assert len(pending) == 1
