...
We stub :class:`RemoteEmbedder` and :class:`QdrantClient`
...
import core.brain.memory.semantic_memory as sm_mod
...
def stub_external_clients(monkeypatch: pytest.MonkeyPatch) -> dict[str, MagicMock]:
    """Replace RemoteEmbedder + QdrantClient so __init__ is offline.

    Returns a dict so individual tests can assert on call args (the
    RemoteEmbedder mock receives the resolved model name, which
    ...
    monkeypatch.setattr(sm_mod, "RemoteEmbedder", encoder_factory)
    monkeypatch.setattr(sm_mod, "QdrantClient", qdrant_factory)
...
def test_default_construction_uses_pre_sprint_a_model(
    ...
    stub_external_clients["encoder"].assert_called_once_with(model=DEFAULT_EMBEDDING_MODEL)
...
def test_env_var_drives_model_and_known_dim(
    ...
    stub_external_clients["encoder"].assert_called_once_with(model=SPRINT_A_EMBEDDING_MODEL)
...
def test_explicit_kwargs_override_env(
    ...
    stub_external_clients["encoder"].assert_called_once_with(model="custom-model")
