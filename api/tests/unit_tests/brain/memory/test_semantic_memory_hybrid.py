...
:class:`core.brain.memory.semantic_memory.SemanticMemory.search`
...
from core.brain.memory.semantic_memory import (
    MemoryRecord,
    SemanticMemory,
)
...
def _stub_external_clients(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ``SemanticMemory.__init__`` instantiates an ``QdrantClient``
    (which spawns a compatibility-check background thread) and a
    ``RemoteEmbedder`` (which loads weights from the HuggingFace
    hub).  Neither is acceptable in a unit test — the patches below
    swap both for ``MagicMock`` so construction is instant and
    side-effect free.
    """
    monkeypatch.setattr(
        "core.brain.memory.semantic_memory.QdrantClient",
        MagicMock,
    )
    monkeypatch.setattr(
        "core.brain.memory.semantic_memory.RemoteEmbedder",
        MagicMock,
    )
...
def test_search_uses_dense_when_flag_off() -> None:
    ...
    result = sm.search(query="hello", top_k=3)

    assert [r.id for r in result] == ["r0", "r1", "r2"]
    sm._dense_search.assert_called_once()  # type: ignore[attr-defined]
    sm._sparse_search.assert_not_called()  # type: ignore[attr-defined]


def test_search_uses_dense_when_no_sparse_encoder() -> None:
    ...
    result = sm.search(query="hello", top_k=2)

    assert len(result) == 2
    sm._sparse_search.assert_not_called()  # type: ignore[attr-defined]


def test_search_propagates_metadata_filter_to_dense() -> None:
    ...
    call = sm._dense_search.call_args  # type: ignore[attr-defined]
...
def test_search_runs_hybrid_when_flag_on_and_encoder_present(
    ...
    monkeypatch.setattr(
        "core.brain.memory.hybrid_search.reciprocal_rank_fusion",
        _fake_fusion,
    )
    ...
    out = sm.search(query="hello", top_k=2)

    assert len(out) == 2  # _fake_fusion truncates dense pool to top_n
    assert sm._sparse_search.call_count == 1  # type: ignore[attr-defined]
    assert sm._dense_search.call_count == 1  # type: ignore[attr-defined]
    assert fusion_calls[0]["k"] == 2
    # Pool is wider than top_k for fusion headroom.
    assert sm._dense_search.call_args.kwargs["top_k"] == 4


def test_search_falls_back_to_dense_on_sparse_failure() -> None:
    ...
    assert len(result) == 3
    assert sm._dense_search.call_count == 2  # type: ignore[attr-defined]


def test_sparse_search_raises_until_migration() -> None:
    """Stub method must surface a clear error pointing to the ticket."""
    sm = SemanticMemory(sparse_encoder=object())
    with pytest.raises(
        NotImplementedError,
        match="sparse-vector named-config",
    ):
        sm._sparse_search(
            query="x",
            top_k=1,
...
def test_pre_task5_dense_only_signature_preserved() -> None:
