...
_T0 = datetime(2026, 5, 11, tzinfo=UTC)
...
def _synthetic_window(
    *,
    ...
    for _ in range(n):
        confidence = max(0.01, min(0.99, rng.gauss(0.7, 0.15)))
...
def test_empty_window_returns_unit_set() -> None:
    ...
    assert result.labels == frozenset({ConformalLabel.SUCCESS, ConformalLabel.FAILURE})
...
def test_single_label_window_returns_unit_set() -> None:
    ...
    calibrator = MapieSplitConformalCalibrator()
    all_success = [_sample(confidence=0.9, success=True) for _ in range(30)]
    result = calibrator.predict_set(
        samples=all_success,
        model_confidence=0.5,
    )
