...
from core.brain.patterns.pattern_miner import PatternMiner
...
def test_distinct_action_types_produce_distinct_pattern_ids(
    ...
    cases = [_make_case(action=DecisionType.INFORM) for _ in range(5)] + [
...
def test_defer_rationale_uses_explicit_phrasing(
    ...
    cases = [_make_case(action=DecisionType.DEFER) for _ in range(5)]
    rules, _ = miner.mine(cases)
    rule = rules[0]
