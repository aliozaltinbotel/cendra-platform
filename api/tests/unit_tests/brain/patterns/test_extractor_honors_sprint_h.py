...
(``core.brain.patterns.scenario_features.SCENARIO_FEATURES``) and
...
# Port note: the kernel ships an EMPTY SCENARIO_FEATURES mapping (the
# curated whitelists are hospitality pack data — see
# packs/hospitality/scenario_features.yaml).  These tests install the
# pack-equivalent entries onto the synthesiser module, mirroring what
# the Batch 6 pack loader will do at runtime.
_TIMING_OCCUPANCY_WHITELIST = FeatureWhitelist(
    pms_keys=(
        "stage",
...
_PACK_SCENARIO_FEATURES = {
    "access_code_release": _TIMING_OCCUPANCY_WHITELIST,
    "late_checkout": _TIMING_OCCUPANCY_WHITELIST,
    "early_checkin": _TIMING_OCCUPANCY_WHITELIST,
}


@pytest.fixture(autouse=True)
def _reset_scenario_features_flag(monkeypatch) -> Iterator[None]:
    """Scope the Sprint H flag per test and install the pack whitelists."""
    from core.brain.patterns import condition_synthesizer

    monkeypatch.setattr(condition_synthesizer, "SCENARIO_FEATURES", _PACK_SCENARIO_FEATURES)
...
def _seed(cases: list[DecisionCase]) -> InMemoryDecisionCaseStore:
...
def test_extractor_collect_drops_spurious_fields_when_flag_on() -> None:
    ...
    for spurious in ("currency", "source", "status", "total_price"):
        assert spurious not in features, f"{spurious} leaked through extractor — Sprint H wiring regressed"
...
def test_extract_path_drops_spurious_fields_when_flag_on() -> None:
    ...
    inform_rule = next(rule for rule in result.rules if rule.action.action_type is DecisionType.INFORM)
    ...
    assert not spurious, f"spurious fields leaked into rule: {spurious}"
