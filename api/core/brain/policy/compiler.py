...
2. A mapping ``owner_id → derived style id`` that the registry of
   :mod:`core.brain.policy.registry` uses to satisfy the
   :class:`core.brain.planning.OwnerStyleResolver` Protocol.
...
from core.brain.planning.styles import (
    BUILTIN_STYLE_SPECS,
    PlannerStyleId,
    PlannerStyleSpec,
)
...
class OwnerPolicyCompiler:
    ...
    def _build_spec(
        ...
        return PlannerStyleSpec(
            ...
            description=(
                f"{base.description} (owner override {block.owner_id!r}: +{len(block.forbid)} forbid)"
                if block.forbid
