...
:mod:`core.brain.policy.compiler`.
"""
...
class OwnerPolicyParser:
    ...
    def _build_tree(self, source: str) -> Any:
        ...
        if self._lark is None:
            grammar = resources.files("core.brain.policy").joinpath(_GRAMMAR_RESOURCE).read_text(encoding="utf-8")
    ...
    def _build_owner_block(self, owner_tree: Any) -> OwnerBlock:
        ...
        for stmt in owner_tree.children[1:]:
            ...
            if kind == "style_stmt":
                ...
                forbid.extend(self._read_action_kind(t) for t in stmt.children)
    ...
    def _read_style(stmt: Any) -> PlannerStyleId:
        ...
        try:
            ...
            raise OwnerPolicyCompileError(f"unknown style name {token!r}") from exc

    @staticmethod
    def _read_action_kind(token: Any) -> str:
        # Action kinds are opaque vertical-defined strings (golden rule 4;
        # the reference validated against its CardActionKind enum).
        # Tenant-vocabulary validation happens at policy-save time against
        # the workflow-kind / action registries, not in the parser.
        value = str(token).strip().lower()
        if not value:
            raise OwnerPolicyCompileError("empty action kind")
        return value
    ...
    def _read_numeric(stmt: Any) -> NumericConstraint:
        ...
        try:
            ...
            raise OwnerPolicyCompileError(f"unknown numeric metric {metric_value!r}") from exc
        try:
            op = ComparisonOp(op_value)
        except ValueError as exc:
            raise OwnerPolicyCompileError(f"unknown comparison op {op_value!r}") from exc
        try:
            value = int(str(int_token))
        except ValueError as exc:
            raise OwnerPolicyCompileError(f"expected integer literal, got {int_token!r}") from exc
...
def _strip_quotes(token: Any) -> str:
    ...
    if len(raw) < 2 or raw[0] != '"' or raw[-1] != '"':
        raise OwnerPolicyParseError(f"expected quoted string, got {raw!r}")
    return raw[1:-1]
