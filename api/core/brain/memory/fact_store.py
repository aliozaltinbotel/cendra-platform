...
Usage:
    ...
    result = store.store_facts(facts, property_id="PROP001")
    matched = store.search("allergic to cats", property_id="PROP001")
"""
...
class FactStore:
    ...
    def get_all(
        ...
        try:
            ...
            results = self._scroll(client, property_id, limit)
            return [self._payload_to_fact(r) for r in results]
        except Exception:
    ...
    def delete(self, fact_id: str) -> bool:
        ...
        try:
            ...
            self._delete_point(client, fact_id)
            return True
        except Exception:
    ...
    def _is_duplicate(
        ...
        results = self._vector_search(client, vector, top_k=1, filters=filters)
        if not results:
            return False
    ...
    def _upsert(self, fact: StoredFact) -> None:
        """Upsert a single fact into Qdrant."""
        vector = self._get_embedding(fact.content)
        if vector is None:
            return
