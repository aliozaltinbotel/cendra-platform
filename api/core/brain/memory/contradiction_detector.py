...
Usage:
    ...
    result = detector.check("Check-in time is 2 PM", property_id="PROP001")
...
ReliabilityResolver = "Callable[[StoredFact | str, bool], SourceReliability | None]"
...
class ContradictionDetector:
    ...
    def check(
        ...
        candidates = self._find_candidates(new_content, property_id)
        if not candidates:
            return DetectionResult()

        # Step 2: pairwise contradiction check
        pairs = self._check_pairs(new_content, candidates)
    ...
    def check_and_apply(
        ...
        """
        result = self.check(new_content, property_id, new_timestamp)

        if result.resolution == Resolution.NEWER_WINS:
            for fact_id in result.superseded_fact_ids:
                deleted = self._store.delete(fact_id)
                if deleted:
    ...
    def _check_pairs(
        ...
        for candidate in candidates:
            pair = self._compare_single(new_content, candidate)
            pairs.append(pair)
    ...
    def _compare_single(
        ...
        if self._llm is not None:
            return self._compare_with_llm(new_content, existing)
    ...
    def _compare_heuristic(
        ...
        is_contradiction = new_has_neg != old_has_neg and significant_overlap
    ...
    def _resolve(
        ...
        low_confidence = any(c.confidence < self._confidence_cutoff for c in contradictions)
    ...
    def _new_loses_reliability(
        ...
        * the new fact's tier is strictly less reliable than every
        ...
        return all(new_rank > reliability_rank(tier) for tier in existing_tiers)
