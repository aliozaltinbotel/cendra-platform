...
:mod:`core.brain.certificates.verifier`) reconstructs the same
...
(Batch 5) loads it from the ``BRAIN_AUTONOMY_CERT_KEY`` setting via
``configs.dify_config``; tests pass an explicit key.
"""
    ...
    def __init__(
        ...
        *,
        ...
        if len(signing_key) < MIN_KEY_BYTES:
            raise ValueError(f"signing_key must be at least {MIN_KEY_BYTES} bytes; got {len(signing_key)}")
