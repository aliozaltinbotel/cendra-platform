...
from core.brain.cognition.friction import RewardSimulator
...
class LookupRewardSimulator:
    ...
    def __init__(
        ...
        *,
        ...
        if default_reward != default_reward or default_reward in (float("inf"), float("-inf")):
...
class GRPOTrainer:
    ...
    def step(
        ...
        advantages = {kind: rewards[kind] - baseline for kind in rewards}
        for kind in weights.kinds():
            advantage = advantages[kind]
            scaled = self._lr * advantage
            weights.bias[kind] = weights.bias[kind] + scaled - self._lr * self._l2 * weights.bias[kind]
        ...
        objective = sum(probs[kind] * rewards[kind] for kind in rewards)
        ...
        for epoch in range(iterations):
            ...
            for features in contexts:
                ...
                for value in advantages.values():
                    ...
                    advantage_count += 1
            logger.info(
                "grpo.epoch epoch=%s objective=%s",
                epoch + 1,
                round(last_objective, 6),
            )
        mean_abs = total_advantage / advantage_count if advantage_count else 0.0
        return GRPOMetrics(
