...
This module ships the pure-Python seam for that loop:
  ...
  * :class:`CritiqueReport` — value object carrying the verbal
    ...
    that downstream consumers (notably :class:`~core.brain.
    cognition.friction.FrictionTracker`) can absorb as
...
not.  See :mod:`core.brain.cognition.friction` for the
matching half.
...
------------
  ...
  * Trajectories are caller-supplied; this module does not
    ...
    typically nightly, alongside :func:`~core.brain.
    cognition.sleep.summarise_decisions`).
"""
...
class CritiqueReport:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0.0 <= self.dissatisfaction <= 1.0:
            raise ValueError("dissatisfaction must be in [0, 1]")
        for kind, value in self.avoidance_hints.items():
            if value != value or value in (
                float("inf"),
                float("-inf"),
            ):
                raise ValueError(f"avoidance hint must be finite for {kind!r}")
            if not 0.0 <= value <= 1.0:
                raise ValueError(f"avoidance hint must be in [0, 1] for {kind!r}")
...
class ReflexionCritic:
    ...
    def critique(
        ...
        logger.info(
            "critic.report n=%s mean_reward=%s dissatisfaction=%s",
            len(events),
            round(mean_reward, 4),
            round(dissatisfaction, 4),
        )
        return CritiqueReport(
    ...
    def _rank_worst_features(
        ...
        for name in sorted(feature_names):
            present = [e.reward for e in events if e.features.get(name, 0.0) > 0.0]
            absent = [e.reward for e in events if e.features.get(name, 0.0) <= 0.0]
            if not present:
                continue
            present_mean = sum(present) / len(present)
            absent_mean = sum(absent) / len(absent) if absent else 0.0
    ...
    def _build_avoidance_hints(
        ...
        *,
        ...
        for event in events:
            per_kind_rewards.setdefault(event.chosen_kind, []).append(event.reward)
    ...
    def _verbalise_reflection(
        ...
        *,
        ...
        if dissatisfaction < self._verbalise:
            return ""
        parts = [f"Mean reward {mean_reward:+.3f} over {sample_size} step(s); dissatisfaction {dissatisfaction:.2f}."]
        if worst:
            named = ", ".join(f"{w.name} (gap {w.gap:+.3f})" for w in worst)
            parts.append(f"Features correlated with punishment: {named}.")
        if hints:
            ranked = sorted(hints.items(), key=lambda kv: (-kv[1], kv[0]))
            named = ", ".join(f"{kind.value} ({weight:.2f})" for kind, weight in ranked)
            parts.append(f"Avoid on future similar contexts: {named}.")
        return " ".join(parts)
