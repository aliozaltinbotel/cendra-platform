...
NEVER_AUTO_SCENARIOS: Final[frozenset[str]] = frozenset(
    {
        "damage_report",
        ...
        "proxy_booking_risk",  # fraud / identity risk
        "safety_emergency",  # gas, fire, medical, intruder
        "safety_security_concern",  # lock failure, pest, safety
        "chargeback_dispute",  # financial / legal exposure
        "privacy_concern",  # surveillance, GDPR
        "off_platform_contact",  # off-OTA / fraud vector
    }
)
...
class ValidationContext:
    ...
    :mod:`core.brain.blockers`, :mod:`core.brain.staticity`, …),
    ...
    complaint_contamination_threshold: float = _COMPLAINT_CONTAMINATION_THRESHOLD
...
class PatternValidator:
    ...
    def validate(
        ...
        *,
        ...
        if not self._check_min_support(rule):
            failures.append(
                f"Insufficient support: {rule.support_count} < {self._min_support} required.",
            )
        ...
        if not self._check_not_blacklisted(rule):
            failures.append(
                f"Scenario '{rule.scenario}' or action is in NEVER_AUTO_LEARN blacklist. Manual rules only.",
            )

        if not self._check_staleness(rule):
            failures.append(
                f"Most recent supporting case is older than {self._max_staleness_days} days. Rule may be outdated.",
            )
        ...
        if failures:
            logger.warning(
                "rule_validation_failed pattern_id=%s scenario=%s failures=%s",
                rule.pattern_id[:8],
                rule.scenario,
                failures,
            )
        ...
        logger.info(
            "rule_validated pattern_id=%s scenario=%s confidence=%s mode=%s",
            rule.pattern_id[:8],
            rule.scenario,
            rule.confidence,
    ...
    def _complaint_recovery_ratio(
        ...
        if not rule.source_case_ids:
            return 0.0
        contaminated = sum(1 for cid in rule.source_case_ids if cid in context.complaint_compensation_case_ids)
