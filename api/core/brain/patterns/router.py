...
inferred by :class:`~core.brain.patterns.classifier.DecisionClassifier`
and a feature dict derived from the live
:class:`~PipelineState (conversation layer — retired, see PORTING_MAP)`, the router
...
from core.brain.patterns.store import PatternRuleStore
...
class PatternRuleRouter:
    ...
    def match(
        ...
        *,
        ...
        Args:
            scenario: str inferred by
                :class:`~core.brain.patterns.classifier.DecisionClassifier`.
    ...
    def _match_in_scope(
        ...
        *,
        ...
        """
        candidates = self._rule_store.get_active_rules(
            scenario=scenario,
            scope=scope,
            scope_id=scope_id,
        )
...
def _is_valid_at(rule: PatternRule, as_of: datetime | None) -> bool:
    ...
    * ``deactivated_at`` IS NULL or > ``as_of`` — Brain Engine had
    ...
    if rule.invalid_at is not None and rule.invalid_at <= as_of:
        return False
    return rule.deactivated_at is None or rule.deactivated_at > as_of
