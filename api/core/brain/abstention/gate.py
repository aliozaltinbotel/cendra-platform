...
class AbstentionGate:
    ...
    def decide(
        ...
        *,
        ...
        if conformal is not None and model_confidence <= conformal:
    ...
    def _insufficient(
        ...
        *,
        ...
        rationale = f"only {sample_size} sample(s); min_samples={self._min_samples}"
        logger.info(
            "abstention.insufficient tool_id=%s sample_size=%s",
            tool_id,
            sample_size,
        )
    ...
    def _abstain_wilson(
        ...
        *,
        ...
        rationale = f"wilson_lb={wilson_lb:.3f} < threshold={self._wilson_threshold:.2f}"
        logger.info(
            "abstention.wilson_below_threshold tool_id=%s wilson_lb=%s threshold=%s",
            tool_id,
            wilson_lb,
            self._wilson_threshold,
        )
        return AbstentionDecision(
            tool_id=tool_id,
    ...
    def _abstain_conformal(
        ...
        *,
        ...
        rationale = (
            f"model_confidence={model_confidence:.3f} <= conformal_threshold={conformal:.3f} (alpha={self._alpha:.2f})"
        )
        logger.info(
            "abstention.conformal_below_threshold tool_id=%s model_confidence=%s conformal=%s alpha=%s",
            tool_id,
            model_confidence,
            conformal,
            self._alpha,
