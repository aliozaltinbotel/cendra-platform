...
class SplitConformalCalibrator:
    ...
    def __init__(
        self,
        *,
        non_conformity: NonConformityFn = (binary_inverse_confidence),
        ...
        *,
        ...
        if conformal.is_singleton:
            ...
            return ConformalAbstainResult(
                ...
                model_confidence=model_confidence,
                rationale=(f"singleton {{{label.value}}}; q={conformal.quantile:.3f}"),
            )
        if conformal.is_empty:
            return ConformalAbstainResult(
                ...
                conformal_set=conformal,
                model_confidence=model_confidence,
                rationale=("empty conformal set; coverage gate fails in both directions"),
            )
        return ConformalAbstainResult(
            verdict=AbstentionVerdict.ABSTAIN,
            conformal_set=conformal,
            model_confidence=model_confidence,
            rationale=(f"ambiguous conformal set {{success, failure}}; both labels survive q={conformal.quantile:.3f}"),
        )
