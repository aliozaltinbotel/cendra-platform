...
:class:`~core.brain.abstention.split_conformal.
...
this module is the *third-party-audited* alternative that:
  ...
  * exposes the same :class:`~core.brain.abstention.
    split_conformal.ConformalSet` value object so downstream
    consumers (notably :class:`~core.brain.abstention.
...
------------
  ...
  * MAPIE 1.4+ API surface
    ...
    ``predict_set``).  Pinned in ``api/pyproject.toml``.
...
class _ConfidencePassthroughClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(
        self,
        X: np.ndarray,  # noqa: N803 - sklearn estimator interface
        y: np.ndarray,
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:  # noqa: N803 - sklearn estimator interface
        """Argmax over the pass-through probabilities."""
        confidences = np.asarray(X, dtype=float).ravel()
        return (confidences >= 0.5).astype(int)

    def predict_proba(self, X: np.ndarray) -> np.ndarray:  # noqa: N803 - sklearn estimator interface
...
class MapieSplitConformalCalibrator:
    ...
    def __init__(
        ...
        *,
        ...
        if not conformity_score:
            raise ValueError("conformity_score must be non-empty")
        self._alpha = alpha
    ...
    def predict_set(
        ...
        *,
        ...
        if X_calib.shape[0] == 0 or unique_labels != {0, 1}:
            return ConformalSet(
                labels=frozenset(
                    {
...
class MapieAbstainGate:
    ...
    Surface mirrors :class:`~core.brain.abstention.
        ...
        *,
        ...
        if len(samples) < self._min_calibration:
            return ConformalAbstainResult(
                ...
                rationale=(f"only {len(samples)} sample(s); min_calibration={self._min_calibration}"),
            )
        ...
        if conformal.is_singleton:
            ...
            return ConformalAbstainResult(
                ...
                model_confidence=model_confidence,
                rationale=(f"mapie singleton {{{label.value}}}; alpha={self._calibrator.alpha:.3f}"),
            )
        if conformal.is_empty:
            return ConformalAbstainResult(
                ...
                conformal_set=conformal,
                model_confidence=model_confidence,
                rationale=("mapie empty conformal set; coverage gate fails in both directions"),
            )
        return ConformalAbstainResult(
            verdict=AbstentionVerdict.ABSTAIN,
            conformal_set=conformal,
            model_confidence=model_confidence,
            rationale=("mapie ambiguous conformal set {success, failure}; both labels survive"),
        )
