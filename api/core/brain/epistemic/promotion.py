...
class PromotionRefusal(RuntimeError):  # noqa: N818 - reference API name kept for porting parity
    ...
    def __init__(
        ...
        *,
        ...
        message = f"refused to promote {subject!r}: {reason}; n={sample_size} wilson={wilson_lb:.3f}"
...
class BeliefPromotionGate:
    ...
    def __init__(
        ...
        *,
        ...
        if not 0.0 <= wilson_threshold <= 1.0:
            raise ValueError("wilson_threshold must be in [0.0, 1.0]")
        if z <= 0.0:
    ...
    def evaluate(
        ...
        *,
        ...
        successes = sum(1 for obs in observations if predicate(obs))
        ...
        *,
        ...
        for obs in observations:
            if obs.subject != subject:
                raise ValueError(f"observation subject {obs.subject!r} != requested subject {subject!r}")
        ...
        if sample_size < self._min_samples:
            raise PromotionRefusal(
                ...
                wilson_lb=wilson_lb,
                reason=(f"sample_size < min_samples={self._min_samples}"),
            )
        if wilson_lb < self._wilson_threshold:
            raise PromotionRefusal(
                ...
                sample_size=sample_size,
                wilson_lb=wilson_lb,
                reason=(f"wilson_lb < threshold={self._wilson_threshold:.2f}"),
            )
        identifier = belief_id or secrets.token_urlsafe(16)
        instant = promoted_at or datetime.now(UTC)
        ...
        return Belief(
            ...
            wilson_lb=wilson_lb,
            sample_size=sample_size,
            supporting_observation_ids=tuple(obs.observation_id for obs in observations),
            promoted_at=instant,
