...
provider in ``brain_engine.security``) and stay inside the provider's
...
class KeyDerivation:
    ...
    def derive(
        ...
        *,
        ...
        block = hmac.new(
            prk, info + b"\x01", hashlib.blake2b,
        ).digest()
...
class EnvKeyProvider:
    ...
    def key_for(self, purpose: str) -> KeyHandle:
        ...
        sub_key = KeyDerivation.derive(
            master_key=master, purpose=purpose, salt=salt,
        )
...
class Encryptor(Protocol):
    ...
    def encrypt(
        self, *, plaintext: bytes, key: KeyHandle, aad: bytes = b"",
    ) -> bytes:
        """Return a self-contained ciphertext (nonce ‖ ct ‖ tag)."""

    def decrypt(
        self, *, ciphertext: bytes, key: KeyHandle, aad: bytes = b"",
    ) -> bytes:
...
class AESGCMEncryptor:
    ...
    def _load_aesgcm() -> type:
        try:
            ...
            raise RuntimeError(
                "cryptography is required for AESGCMEncryptor; "
