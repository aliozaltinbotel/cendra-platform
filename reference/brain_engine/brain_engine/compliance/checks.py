...
- :func:`never_ai_action` — wraps the structurally-banned action
  set from :mod:`brain_engine.compliance.never_ai_denylist`.
...
from brain_engine.compliance.never_ai_denylist import (
    NeverAICategory,
    is_never_ai,
    reason_for,
)
...
REG_2024_1028_BOOKING_KINDS: Final[frozenset[CardActionKind]] = (
    frozenset(
        {
...
_HITL_REQUIRED_KINDS: Final[frozenset[CardActionKind]] = frozenset(
    {
...
def hitl_required_for_high_risk(
    ...
    return ComplianceViolation(
        ...
        reason=(
            "EU AI Act Art. 14 requires explicit human oversight "
            "before this action class is executed."
        ),
        evidence={"action_kind": context.action_kind.value},
    )
...
def jurisdiction_min_nights(
    ...
    if context.action_kind is not CardActionKind.CONFIRM_BOOKING:
        return None
    ...
    return ComplianceViolation(
        ...
        severity=ComplianceSeverity.BLOCK,
        reason=(
            f"jurisdiction {juris} requires at least "
            f"{floor} nights; booking covers {nights}."
        ),
        evidence={
            "jurisdiction": juris,
            "floor": str(floor),
