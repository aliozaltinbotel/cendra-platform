...
The record is built on top of :mod:`brain_engine.evidence.audit_pack`'s
...
- ``action_kind`` — canonical
  :class:`brain_engine.cards.action_kinds.CardActionKind` value.
- ``autonomy_tier`` — the
  :class:`brain_engine.certificates.AutonomyTier` the action ran
  at (Moat #3).  ``None`` if certificates are not yet wired.
- ``planner_style`` — the
  :class:`brain_engine.planner.PlannerStyleId` that shaped the
...
class Art12Decision:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.prev_digest) != 64:
            raise ValueError(
                "prev_digest must be 64 hex chars (BLAKE2B-256)"
            )
...
def canonical_record(decision: Art12Decision) -> bytes:
    ...
    payload["action_kind"] = decision.action_kind.value
...
def chained_digest(decision: Art12Decision) -> str:
    ...
    return hashlib.blake2b(
        canonical_record(decision), digest_size=32,
    ).hexdigest()
