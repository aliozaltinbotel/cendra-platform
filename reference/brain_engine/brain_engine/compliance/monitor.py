...
class ComplianceMonitor:
    ...
    def __init__(
        ...
        *,
        ...
        self._log = logger.bind(component="compliance_monitor")
    ...
    def _aggregate_kind(
        ...
        if any(
            v.severity is ComplianceSeverity.BLOCK
            for v in violations
        ):
            return VerdictKind.BLOCKED
        if any(
            v.severity is ComplianceSeverity.REVIEW
            for v in violations
        ):
    ...
    def _rationale(
        ...
        top = ordered[0]
        return (
            f"{kind.value}: {top.rule_id} ({top.severity.value}) "
            f"— {top.reason}"
        )
