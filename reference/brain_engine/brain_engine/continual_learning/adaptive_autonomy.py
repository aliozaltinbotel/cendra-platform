...
accumulates.  See :mod:`brain_engine.patterns.wilson`.
"""
...
class AdaptiveAutonomyManager:
    ...
    async def get_level(
        self, owner_id: str, property_id: str,
    ) -> AutonomyLevel:
        ...
        """
        state = await self._load(owner_id, property_id)
        return AutonomyLevel(state.level)

    async def should_ask_approval(
        self,
        owner_id: str,
        property_id: str,
        action: str,
        ...
        level = await self.get_level(owner_id, property_id)
    ...
    async def record_decision(
        ...
        if new_level > state.level:
            logger.info(
                ...
                owner_id, property_id, state.level, new_level,
            )
        ...
        if demoted_level < state.level:
            logger.info(
                ...
                owner_id, property_id, state.level, demoted_level,
            )
    ...
    def _next_level_info(state: OwnerAutonomy) -> dict[str, Any]:
        ...
        thresholds = {
            AutonomyLevel.L1_SUGGEST: (
                _MIN_DECISIONS_FOR_L2, _MIN_WILSON_FOR_L2,
            ),
            AutonomyLevel.L2_ACT_INFORM: (
                _MIN_DECISIONS_FOR_L3, _MIN_WILSON_FOR_L3,
            ),
            AutonomyLevel.L3_SILENT: (
                _MIN_DECISIONS_FOR_L4, _MIN_WILSON_FOR_L4,
            ),
        }
    ...
    async def _load(
        ...
        data = json.loads(raw)
        return OwnerAutonomy(**{
            k: v for k, v in data.items()
            if k in OwnerAutonomy.__dataclass_fields__
        })

    async def _save(self, state: OwnerAutonomy) -> None:
