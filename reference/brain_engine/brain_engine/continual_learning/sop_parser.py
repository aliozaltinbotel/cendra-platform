...
class SOPParser:
    ...
    async def parse_and_store(
        ...
        """
        rules = await self._extract_with_llm(sop_text)
        if not rules:
            rules = self._extract_with_patterns(sop_text)

        created: list[dict[str, Any]] = []
        for rule_data in rules:
            proc = await self._memory.store_manual_rule(
    ...
    async def _extract_with_llm(
        ...
        try:
            ...
            response = await litellm.acompletion(
                ...
                messages=[
                    {"role": "system", "content": _SOP_EXTRACTION_SYSTEM},
                    {"role": "user", "content": sop_text[:3000]},
                ],
                temperature=0.1,
                max_tokens=1500,
