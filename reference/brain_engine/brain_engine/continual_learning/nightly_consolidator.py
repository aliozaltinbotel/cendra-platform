...
class NightlyConsolidator:
    ...
    async def run_nightly(self) -> dict[str, Any]:
        ...
        stats["step1_memory"] = await self._step1_consolidate_memory()
        stats["step2_skills"] = await self._step2_evolve_skills()
        stats["step3_preferences"] = await self._step3_aggregate_preferences()
        stats["step4_knowledge_graph"] = await self._step4_update_knowledge_graph()
        stats["step5_pruning"] = await self._step5_prune_skills()
        ...
        stats["step6_mira"] = await self._step6_mira_ready()
        stats["step7_patterns"] = await self._step7_extract_patterns()
        ...
        if self._golden_cases_runner is not None and golden_cases_enabled():
            stats["step8_evaluation"] = await self._step8_run_evaluation()
        ...
        stats["step9_foundation_drift"] = (
            await self._step9_detect_foundation_drift()
        )

        logger.info("=== Nightly Consolidation Complete: %s ===", stats)
        return stats

    async def _step8_run_evaluation(self) -> dict[str, Any]:
        ...
        try:
            ...
            report = await self._golden_cases_runner.run_daily()
            return {
    ...
    async def _step1_consolidate_memory(self) -> dict[str, Any]:
        ...
        try:
            result = await self._memory.consolidator.consolidate(force=True)
        ...
        fact_stats = await self._extract_and_store_facts()
        ...
        stats["episodic_dedup"] = await self._dedup_recent_episodes()

        # Confidence decay (unchanged)
        await self._decay_skill_confidence()
        stats["confidence_decay_applied"] = True

        return stats

    async def _dedup_recent_episodes(self) -> dict[str, Any]:
        ...
        try:
            episodes = await self._memory.episodic.get_recent(
        ...
        for episode in episodes:
            try:
                vector = encoder.encode(
                    episode.content, normalize_embeddings=True,
                )
    ...
    async def _extract_and_store_facts(self) -> dict[str, Any]:
        ...
        try:
            ...
            recent_episodes = await self._memory.episodic.get_recent(n=100)
        except Exception:
        ...
        for user_id, messages in conversations.items():
            try:
                facts = await self._mem0.extract_facts(
                ...
                stored = await self._filter_contradicting_facts(stored)

                result = await self._fact_store.store_facts(stored)
                ...
                logger.warning(
                    "Fact extraction failed for user %s", user_id,
                    exc_info=True,
                )
                store_result_totals["errors"] += 1

        logger.info(
            "Step 1 fact extraction: %d conversations, %d facts found, "
            "%d stored, %d duplicates, %d contradictions",
            len(conversations),
            total_extracted,
    ...
    async def _filter_contradicting_facts(
        self,
        ...
        """
        if (
            self._contradiction_detector is None
            or not _contradiction_check_enabled()
        ):
        ...
        for fact in candidates:
            ...
            try:
                detection = await self._contradiction_detector.check(
                    new_content=fact.content,
                    property_id=fact.entity_id,
                    new_timestamp=fact.created_at.isoformat()
                    if fact.created_at
                    else "",
                )
            except Exception:
    ...
    def _group_episodes_to_conversations(
        ...
        for ep in episodes:
            user_id = (
                getattr(ep, "guest_id", None)
                or getattr(ep, "user_id", None)
                or "unknown"
            )
    ...
    async def _decay_skill_confidence(self) -> None:
        ...
        """
        all_skills = await self._memory.procedural.get_all_procedures(
            active_only=True,
        )
        for skill in all_skills:
            ...
            if abs(old_conf - skill.confidence) > 0.001:
                await self._memory.procedural._redis.set(
    ...
    async def _step2_evolve_skills(self) -> dict[str, Any]:
        ...
        try:
            failures = await self._recorder.get_failures(since)
        except Exception:
        ...
        for event_type, group in grouped.items():
            result = await self._evolve_from_failure_group(
                event_type, group,
            )
            if result == "evolved":
                evolved += 1
    ...
    async def _evolve_from_failure_group(
        self,
        event_type: str,
        failures: list[Any],
    ) -> str:
        ...
        try:
            result = await self._skills.evolve_on_failure(
                event_type=event_type,
                event_description=getattr(
                    representative, "input_message", "",
                ),
                action_taken=str(
            ...
            logger.error(
                "Step 2: evolution failed for %s", event_type,
                exc_info=True,
            )
            return "error"
    ...
    async def _step3_aggregate_preferences(self) -> dict[str, Any]:
        ...
        try:
            approvals = await self._recorder.get_approvals(since)
        except Exception:
        ...
        rules_created = await self._create_rules_from_approvals(approvals)
    ...
    async def _create_rules_from_approvals(
        ...
        for event_type, counts in patterns.items():
            ...
            rule = await self._create_preference_rule(
                event_type, counts, total,
            )
            if rule:
                rules_created += 1
    ...
    async def _create_preference_rule(
        self,
        event_type: str,
        counts: dict[str, int],
        total: int,
        ...
        if approve_rate >= _APPROVAL_CONSISTENCY:
            return await self._memory.procedural.add_procedure(
    ...
    async def _step4_update_knowledge_graph(self) -> dict[str, Any]:
        ...
        # ── Deterministic path ─────────── #
        if (
            self._deterministic_kg_sync is not None
            and deterministic_sync_enabled()
            and self._case_store is not None
        ):
            try:
                cases = await self._case_store.search(limit=200)
                sync_stats = await (
                    self._deterministic_kg_sync.sync_decision_cases(cases)
                )
                stats["deterministic"] = {
                    ...
                    "relationships_written": (
                        sync_stats.relationships_written
                    ),
                }
        ...
        if run_llm:
            try:
                recent = await self._memory.episodic.get_recent(n=50)
                if recent:
                    result = await (
                        self._memory.consolidator.consolidate(force=True)
                    )
                    stats["llm"] = {
    ...
    async def _step5_prune_skills(self) -> dict[str, Any]:
    ...
    async def _step6_mira_ready(self) -> dict[str, Any]:
    ...
    async def _step7_extract_patterns(self) -> dict[str, Any]:
        ...
        scopes = await self._collect_extraction_scopes()
        stats["scenarios_checked"] = len(scopes)

        for scenario, property_id, owner_id in scopes:
            try:
                result = await self._pattern_extractor.extract_patterns(
                    scenario=scenario,
        ...
        logger.info(
            "Step 7 pattern extraction: %d scopes, %d extracted, "
            "%d validated, %d stored",
    ...
    async def _step9_detect_foundation_drift(self) -> dict[str, Any]:
        ...
        if (
            ...
            or self._foundation_update_store is None
        ):
            return {
                "skipped": True,
        ...
        try:
            ...
            cases = await self._case_store.search(limit=5000)
            stats["cases_scanned"] = len(cases)
            candidates = detect_foundation_drift(cases)
            for candidate in candidates:
                try:
                    await self._foundation_update_store.upsert(candidate)
        ...
        logger.info(
            "Step 9 foundation drift: %d cases scanned, "
            "%d candidates emitted, %d errors",
            stats["cases_scanned"],
    ...
    async def _collect_extraction_scopes(
        self,
        ...
        for scenario in Scenario:
            ...
            count = await self._case_store.count(scenario=scenario)
            if count < 3:
                continue

            cases = await self._case_store.search(
                scenario=scenario, limit=1,
            )
            if cases:
                sample = cases[0]
                scopes.append((
                    scenario, sample.property_id, sample.owner_id,
                ))
    ...
    async def run_monthly(self) -> dict[str, Any]:
        ...
        metrics = await self._compute_monthly_metrics()
        cleanup = await self._monthly_deep_cleanup()

        report = {**metrics, "cleanup": cleanup}
        logger.info("=== Monthly Evaluation Complete: %s ===", report)
        return report

    async def _compute_monthly_metrics(self) -> dict[str, Any]:
        ...
        """
        try:
            graded = await self._recorder.get_graded(days=30)
        except Exception:
        ...
        interventions = sum(
            1 for i in graded if getattr(i, "owner_intervened", False)
        )
        self_resolved = sum(
            1 for i in graded
        ...
        event_counts = Counter(
            getattr(i, "event_type", "unknown") for i in graded
        )
        ...
        return {
            ...
            "skills_total": await self._memory.procedural.count(),
            "top_event_types": dict(event_counts.most_common(10)),
        }

    async def _monthly_deep_cleanup(self) -> dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self._recorder, "cleanup_expired"):
                expired_cleaned = await self._recorder.cleanup_expired(
                    max_age_days=90,
                )
