...
class InteractionRecorder:
    ...
    async def record(self, interaction: BrainEngineInteraction) -> str:
    ...
    async def update_outcome(
        ...
        """
        interaction = await self._get(interaction_id)
        if not interaction:
            logger.warning("Interaction %s not found for update", interaction_id)
            return

        self._apply_outcome_fields(
            interaction, grader_score, owner_approved,
            guest_satisfied, owner_intervened,
            resolved_without_escalation, response_time_minutes,
            cascade_level,
        )

        await self._save_and_reindex(interaction)
    ...
    async def _save_and_reindex(
        self, interaction: BrainEngineInteraction,
    ) -> None:
    ...
    async def get_failures(
        ...
        """
        return await self._get_from_sorted_set(
            f"{_PREFIX}failures", since,
        )
    ...
    async def get_graded(
        ...
        all_recent = await self._get_from_sorted_set(
            f"{_PREFIX}timeline", since,
        )
    ...
    async def get_by_property(
        ...
        since = datetime.now(timezone.utc) - timedelta(days=days)
        return await self._get_from_sorted_set(
            f"{_PREFIX}by_prop:{property_id}", since,
        )
    ...
    async def get_by_event_type(
        ...
        since = datetime.now(timezone.utc) - timedelta(days=days)
        return await self._get_from_sorted_set(
            f"{_PREFIX}by_type:{event_type}", since,
        )
    ...
    async def count(self, days: int = 30) -> int:
        ...
        return await self._redis.zcount(
            f"{_PREFIX}timeline", since_ts, now_ts,
        )

    async def count_failures(self, days: int = 30) -> int:
        ...
        now_ts = datetime.now(timezone.utc).timestamp()
        return await self._redis.zcount(
            f"{_PREFIX}failures", since_ts, now_ts,
        )
    ...
    async def cleanup_expired(self, max_age_days: int = 90) -> int:
        ...
        sets_to_clean = await self._get_sorted_set_keys()
        total_removed = 0
    ...
    async def _get_sorted_set_keys(self) -> list[str]:
        ...
        while True:
            cursor, found = await self._redis.scan(
                cursor=cursor, match=f"{_PREFIX}by_*", count=100,
            )
            keys.extend(found)
    ...
    async def _get(
        ...
        """
        raw = await self._redis.get(f"{_PREFIX}{interaction_id}")
        if not raw:
            return None
        return _deserialize(raw)

    async def _get_from_sorted_set(
        ...
        return await self._batch_get(ids)
...
def _deserialize(raw: str) -> BrainEngineInteraction | None:
    ...
    try:
        ...
        return BrainEngineInteraction(**{
            ...
            if k in BrainEngineInteraction.__dataclass_fields__
        })
