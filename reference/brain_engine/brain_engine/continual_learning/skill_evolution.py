...
class SkillEvolutionEngine:
    ...
    async def evolve_on_failure(
        ...
        existing = await self._memory.find_best_match(event_type, context)

        # REFLECT: LLM analyzes the failure
        reflection = await self._reflect(
            event_description, action_taken, failure_reason,
            existing, context,
        )

        # WRITE: create or update the skill
        if existing:
            skill = await self._update_skill(existing, reflection, event_type)
        else:
            skill = await self._create_skill(event_type, reflection)
        ...
        if skill is None:
            result = SkillEvolutionResult(
                status="blocked",
                change_description=(
                    "Blocked: contradicts manual/immutable rule"
                ),
        ...
        result = await self._verify(skill, reflection)
        self._log_evolution(event_type, result)
        return result

    # ── Public: Reinforce on Success ────────────────────────────────── #

    async def evolve_on_success(
        self,
        event_type: str,
        context: dict[str, Any],
    ) -> SkillEvolutionResult:
    ...
    async def _update_skill(
        ...
        evolution_note = (
            f" [Evolved {datetime.now(timezone.utc).strftime('%Y-%m-%d')}:"
            f" {reflection.summary[:100]}]"
        )
        ...
        logger.info(
            ...
            skill.name, skill.confidence, skill.actions,
        )
        return skill
    ...
    async def _check_manual_conflict(
        self,
        event_type: str,
        actions: list[str],
    ) -> str:
        ...
        """
        all_procs = await self._memory.get_all_procedures()
        for proc in all_procs:
            ...
            if proc.source in ("manual", "immutable") and proc_category == event_type:
                ...
                if rule_text:
                    return (
                        f"Manual/immutable rule exists for "
                        f"'{event_type}': '{rule_text[:80]}'"
                    )
    ...
    async def _create_skill(
        ...
        conflict = await self._check_manual_conflict(event_type, actions)
        if conflict:
            logger.info(
                "Skipping learned rule creation — %s", conflict,
            )
            return None
        ...
        skill = await self._memory.add_procedure(
            name=name,
    ...
    async def _verify(
        ...
        Returns:
            SkillEvolutionResult.
        """
        existing_skills = await self._memory.get_all_procedures()
        ...
        for action in getattr(skill, "actions", []):
            try:
                result = self._guardrails.validate_action(
                    action, getattr(skill, "trigger_conditions", {}),
                )
                ...
                pass  # guardrail check is best-effort
    ...
    def _log_evolution(
        self, event_type: str, result: SkillEvolutionResult,
    ) -> None:
