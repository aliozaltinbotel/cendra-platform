...
class _OwnerScope:
    ...
    forbid: frozenset[CardActionKind]
...
class Z3OwnerPolicyVerifier:
    ...
    def __init__(self, document: PolicyDocument) -> None:
        ...
        self._log = logger.bind(
            component="owner_policy_z3_verifier",
        )
    ...
    def verify(
        ...
        *,
        ...
        if (
            scope.jurisdiction is not None
            and jurisdiction is not None
            and scope.jurisdiction != jurisdiction
        ):
    ...
    def constraints_for(
        ...
        return {
            "owner_id": owner_id,
            "forbid": sorted(k.value for k in scope.forbid),
    ...
    def _build_forbidden_witness(
        ...
        *,
        ...
        solver = z3.Solver()
        solver.add(action_var == action_kind.value)
        for forbidden in forbid:
            solver.add(action_var != forbidden.value)
        verdict = solver.check()
        self._log.info(
            "owner_policy.forbidden",
            owner_id=owner_id,
            action_kind=action_kind.value,
            z3_check=str(verdict),
        )
        return OwnerVerifyResult(
            outcome=OwnerVerifyOutcome.FORBIDDEN_ACTION,
            rationale=(
                f"owner {owner_id!r} forbids "
                f"{action_kind.value} (z3.check={verdict})"
            ),
        )
    ...
    def _build_jurisdiction_witness(
        ...
        *,
        ...
        return OwnerVerifyResult(
            ...
            rationale=(
                f"owner {owner_id!r} pins "
                f"jurisdiction={expected!r}; got "
                f"{got!r} (z3.check={verdict})"
            ),
        )
        ...
        *,
        ...
        for constraint in constraints:
            ...
            if str(verdict) == "unsat":
                self._log.info(
                    "owner_policy.numeric_violation",
                    owner_id=owner_id,
                ...
                return OwnerVerifyResult(
                    outcome=(
                        OwnerVerifyOutcome.NUMERIC_VIOLATION
                    ),
                    rationale=(
