...
:class:`brain_engine.patterns.models.DecisionCase` rows into the
...
_DETERMINISTIC_FLAG_ENV: Final[str] = (
    "BRAIN_KG_DETERMINISTIC_SYNC_ENABLED"
)
_LLM_EXTRACTION_FLAG_ENV: Final[str] = (
...
class DeterministicKGSync:
    ...
    async def sync_decision_cases(
        ...
        for case in cases_list:
            try:
                nodes, edges = await self.sync_decision_case(case)
            except Exception as exc:
                # Per-case failure must not stop the batch.
                logger.warning(
                    "kg.deterministic.case_failed case=%s "
                    "type=%s msg=%s",
                    getattr(case, "case_id", "?"),
                    type(exc).__name__,
                    exc,
