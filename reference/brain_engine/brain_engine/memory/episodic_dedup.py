...
class Consolidator(Protocol):
    ...
    def consolidate(
        self, episodes: Sequence[EpisodeRecord],
    ) -> DedupReport: ...
...
class EpisodicDedupConsolidator:
    ...
    def _best_cluster(
        ...
        for cluster in clusters:
            sim = cosine_similarity(
                episode.embedding, cluster.centroid,
            )
            if sim >= best_sim:
                best_sim = sim
                best = cluster
    ...
    def _build_report(
        self, clusters: list["_Cluster"], *, total: int,
    ) -> DedupReport:
        ...
        for cluster in clusters:
            ...
            removed.extend(
                ...
                if m != summary.representative_id
            )
        return DedupReport(
...
class _Cluster:
    ...
    __slots__ = (
        "_centroid", "_members", "_earliest", "_latest", "_representative",
    )
    ...
    def add(self, episode: EpisodeRecord) -> None:
        ...
        for i, x in enumerate(episode.embedding):
            self._centroid[i] = (
                (self._centroid[i] * n_old + x) / n_new
            )
