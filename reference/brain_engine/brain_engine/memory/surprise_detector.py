...
class SurpriseDetector:
    ...
    async def analyze_event(
        ...
        if event == "damage_detected":
            ...
            if guest_id:
                prev_count = await self._get_pattern_count(f"guest:{guest_id}:damage")
                if prev_count > 0:
                    scores.append(0.3)
            ...
            if damage_location and property_id:
                known = await self._get_pattern_count(f"property:{property_id}:damage:{damage_location}")
                if known == 0:
                    scores.append(0.5)
            ...
            guest_id = ctx.get("guest_id", "")
            if guest_id:
                prev_damage = await self._get_pattern_count(f"guest:{guest_id}:damage")
                if prev_damage > 0:
                    scores.append(0.6)
            ...
            guest_id = ctx.get("guest_id", "")
            if guest_id:
                prev = await self._get_pattern_count(f"guest:{guest_id}:late_checkout")
                if prev > 0:
                    scores.append(0.2)
        ...
        await self._record_pattern(event, ctx)
    ...
    async def _record_pattern(self, event: str, context: dict[str, Any]) -> None:
    ...
    async def _get_pattern_count(self, pattern_key: str) -> int:
        """Get historical count for a pattern."""
        val = await self._redis.get(self._prefix + pattern_key)
    ...
    def should_consolidate(
        ...
        """
        return (
            surprise_score >= self._surprise_threshold
            and access_count >= 2
            and retention >= 0.3
        )
