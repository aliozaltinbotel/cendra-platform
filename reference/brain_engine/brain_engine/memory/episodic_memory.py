...
- Redis (for production, with automatic TTL-based expiration)
"""
...
class EpisodicBackend(ABC):
    ...
    @abstractmethod
    async def get_recent(self, n: int, session_id: str | None = None) -> list[Episode]: ...
    ...
    @abstractmethod
    async def get_session_history(self, session_id: str) -> list[Episode]: ...
...
class JsonFileBackend(EpisodicBackend):
    ...
    async def clear(self, session_id: str | None = None) -> None:
        ...
        if session_id:
            self._episodes = [
                e for e in self._episodes if e.session_id != session_id
            ]
        else:
...
class RedisBackend(EpisodicBackend):
    ...
    async def get_recent(self, n: int, session_id: str | None = None) -> list[Episode]:
        ...
        raw_entries = await self._redis.zrevrange(key, 0, n - 1)
...
class EpisodicMemory:
    ...
    async def add_episode(
        ...
        await self._backend.append(episode)
    ...
    async def get_recent(self, n: int = 10) -> list[Episode]:
        ...
        episodes = await self._backend.get_recent(n, session_id=self.session_id)
    ...
    async def search_by_time_range(
        ...
        """
        return await self._backend.get_by_time_range(
            start, end, session_id=self.session_id
        )
    ...
    async def get_session_history(
        ...
        return await self._backend.get_session_history(sid)

    async def clear(self) -> None:
        """Clear all episodes for the current session."""
        await self._backend.clear(session_id=self.session_id)

    def __repr__(self) -> str:
        return (
            f"EpisodicMemory(session_id={self.session_id!r}, "
