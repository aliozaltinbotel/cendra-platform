...
class CertificateVerifier:
    ...
    def __init__(
        ...
        *,
        ...
        self._policy = policy
        self._log = logger.bind(component="cert_verifier")
    ...
    def verify(
        ...
        *,
        ...
        return VerifyResult(
            ...
            rationale=(
                f"cert {cert.cert_id} valid; tier="
                f"{cert.granted_tier.value}"
            ),
        )
        ...
        *,
        ...
        if cert.is_expired(at=at):
            return VerifyResult(
                ...
                rationale=(
                    f"expired at {cert.expires_at.isoformat()}"
                ),
            )
        return None
    ...
    def _scope_checks(
        *,
        ...
        if cert.action_kind is not action_kind:
            return VerifyResult(
                outcome=VerifyOutcome.WRONG_ACTION,
                rationale=(
                    f"cert action {cert.action_kind.value} != "
                    f"requested {action_kind.value}"
                ),
            )
        if cert.property_id != property_id:
            return VerifyResult(
                ...
                rationale=(
                    f"cert property {cert.property_id!r} != "
                    f"requested {property_id!r}"
                ),
            )
        if cert.owner_id != owner_id:
            return VerifyResult(
                ...
                rationale=(
                    f"cert owner {cert.owner_id!r} != "
                    f"requested {owner_id!r}"
                ),
            )
        return None
    ...
    def _policy_check(
        ...
        ceiling: AutonomyTier = self._policy.ceiling_for(
            cert.action_kind
        )
        if tier_rank(cert.granted_tier) > tier_rank(ceiling):
            return VerifyResult(
                ...
                rationale=(
                    f"granted={cert.granted_tier.value} exceeds "
                    f"policy ceiling={ceiling.value} for "
                    f"{cert.action_kind.value}"
                ),
            )
        return None
