...
class EvaluationResultStore(Protocol):
    ...
    async def save_run(self, report: GoldenCasesReport) -> None:
...
class GoldenCasesRunner:
    ...
    async def run_daily(self) -> GoldenCasesReport:
        """Sample, evaluate, aggregate, persist."""
        start = datetime.now(timezone.utc)
        since = start - timedelta(hours=24)

        cases = await self._sample_recent(since=since)
        if not cases:
        ...
        for case in cases:
            try:
                result = await self._judge_case(case)
                scores.append(result.score)
                if result.metadata.get("would_pm_agree"):
                    pm_matches += 1
                if result.metadata.get("hallucination_detected"):
                    hallucinations += 1
                await self._result_store.save_verdict(
                    case.case_id, result, self._judge_model,
                )
            except Exception:
                failed += 1
                logger.exception(
                    "Golden cases: failed to evaluate %s", case.case_id,
                )
        ...
        await self._result_store.save_run(report)
        logger.info("Golden cases run complete: %s", report)
        return report

    async def _sample_recent(self, since: datetime) -> list[DecisionCase]:
