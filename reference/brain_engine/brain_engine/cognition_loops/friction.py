...
:class:`~brain_engine.cognition_loops.critic.Critic`.
...
which wraps any :class:`~brain_engine.cognition_loops.grpo.
RewardSimulator` and multiplies the realised reward by the
...
from brain_engine.cognition_loops.critic import CritiqueReport
from brain_engine.cognition_loops.grpo import RewardSimulator
    ...
    def __init__(
        ...
        *,
        ...
        if critique_weight < 0.0:
            raise ValueError(
                "critique_weight must be non-negative"
            )
        self._alpha = alpha
        self._decay = ema_decay
        self._critique_weight = critique_weight
        self._states: dict[
            tuple[str, MemoryOpKind], FrictionState
        ] = {}
        self._log = logger.bind(component="friction_tracker")
    ...
    def record(
        ...
        *,
        ...
        state = self._states.setdefault(
            (state_key, op_kind), FrictionState()
        )
        if state.count == 0:
            ...
            state.ema_reward = (
                self._decay * reward
                + (1.0 - self._decay) * state.ema_reward
            )
        state.count += 1
    ...
    def simulate(
        ...
        *,
        ...
        raw = self._inner.simulate(
            features=features, op_kind=op_kind,
        )
        multiplier = self._tracker.friction(
            state_key=state_key, op_kind=op_kind,
        )
