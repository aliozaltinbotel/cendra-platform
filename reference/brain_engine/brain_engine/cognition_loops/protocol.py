...
class InteractionProtocol:
    ...
    def __init__(self) -> None:
        self._log = logger.bind(component="cognition_protocol")
    ...
    def resolve(
        ...
        *,
        ...
        if ace_cycle.target != memory_op.target:
            raise ValueError(
                f"target mismatch: ace={ace_cycle.target!r} "
                f"memory={memory_op.target!r}"
            )
        ...
        self._log.info(
            ...
            applied=applied_kind.value,
            ace=ace_cycle.reflector_verdict.value,
            memory=memory_op.kind.value,
        )
    ...
    def _decide(
        ...
        *,
        ...
        if (
            ace_kind is MemoryOpKind.ADD
            and memory_op.kind is MemoryOpKind.DELETE
        ):
            return (
                MemoryOpKind.NOOP,
                "ACE ADD vs Memory-R1 DELETE; defer to "
                "nightly consolidation",
            )
        return (
            memory_op.kind,
            (
                f"divergence ace={ace_kind.value} "
                f"memory={memory_op.kind.value}; "
                "Memory-R1 carries reward signal"
            ),
        )
    ...
    def _infer_ace_kind(cycle: AceCycle) -> MemoryOpKind:
        ...
        """
        if (
            cycle.reflector_verdict is AceVerdict.APPROVE
            and cycle.curator_applied
        ):
