...
class LogitWeights:
    ...
    def zero(cls) -> LogitWeights:
        ...
        bias: dict[MemoryOpKind, float] = {
            kind: 0.0 for kind in MemoryOpKind
        }
        weights: dict[MemoryOpKind, dict[str, float]] = {
            kind: {} for kind in MemoryOpKind
        }
    ...
    def copy(self) -> LogitWeights:
        ...
        return LogitWeights(
            bias=dict(self.bias),
            weights={
                kind: dict(per_feature)
                for kind, per_feature in self.weights.items()
            },
        )
...
def softmax(scores: Mapping[MemoryOpKind, float]) -> dict[
    MemoryOpKind, float,
]:
    ...
    if total == 0.0:
        ...
        n = len(scores)
        return {kind: 1.0 / n for kind in scores}
