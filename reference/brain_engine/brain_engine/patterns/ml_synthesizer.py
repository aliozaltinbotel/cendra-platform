...
The Wilson-based :class:`brain_engine.patterns.condition_synthesizer
...
2. ``dominant_path`` — the conjunctive condition along the most
   ...
   that :class:`brain_engine.patterns.models.PatternRule` already
...
def configured_max_depth() -> int:
    ...
    try:
        ...
        raise ValueError(
            f"{_ML_MAX_DEPTH_ENV} must be a positive integer, "
            f"got {raw!r}",
        ) from exc
    if value <= 0:
        raise ValueError(
...
class MlSynthesizer:
    ...
    :func:`brain_engine.patterns.condition_synthesizer._flatten`).
    """
    ...
    def __init__(
        ...
        *,
        classifier_factory: Callable[
            [int, int], TreeClassifierProtocol,
        ],
...
def build_default_classifier_factory() -> Callable[
    ...
    def factory(
        ...
        logger.info(
            "Building sklearn DecisionTreeClassifier "
            "(max_depth=%d, min_samples_leaf=%d)",
            max_depth,
            min_samples_leaf,
        )
