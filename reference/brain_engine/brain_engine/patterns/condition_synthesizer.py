...
class ConditionCandidate:
    ...
    Attributes:
        ...
        operator: One of ``gte``, ``lte``, ``eq`` — matches the
            ...
            :func:`brain_engine.patterns.models._evaluate_condition`.
...
class ConditionSynthesizer:
    ...
    def __init__(
        ...
        *,
        ...
        self._log = logger.bind(component="condition_synthesizer")
        ...
        *,
        ...
        for _ in range(self._max_conditions):
            ...
            if best is None:
                break
            above_purity += sum(
                1 for c in candidates if c.purity >= self._min_purity
            )
            rejected_low += sum(
                1
                for c in candidates
                if c.purity >= self._min_purity
                and c.target_matched < self._min_support
            )
            chosen.append(best)
...
def _values_for(
    ...
    return tuple(
        record[key] for record in features if record.get(key) is not None
    )
...
def _score_candidate(
    *,
    ...
    target_hits = sum(
        1
        for record in target_features
        if _matches(record.get(key), operator, value)
    )
    other_hits = sum(
        1
        for record in other_features
...
def _matches(actual: Any, operator: str, expected: Any) -> bool:
    ...
    try:
        ...
        if operator == "in":
            return _canonical(actual) in {
                _canonical(value) for value in expected
            }
        if operator == "not_in":
            return _canonical(actual) not in {
                _canonical(value) for value in expected
            }
    except TypeError:
        return False
...
def _quantiles(values: Sequence[float]) -> tuple[float, ...]:
    ...
    probes = [
        _quantile(sorted_values, q) for q in _NUMERIC_QUANTILES
    ]
    seen: list[float] = []
    for value in probes:
...
def _filter_by_candidate(
    ...
    *,
    ...
    target_kept = [
        record
        for record in target_pool
        if _matches(record.get(field_name), operator, value)
    ]
    other_kept = [
        record
        for record in other_pool
        if _matches(record.get(field_name), operator, value)
    ]
