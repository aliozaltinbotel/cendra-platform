...
# :mod:`brain_engine.patterns.condition_synthesizer._PMS_KEYS` /
...
# Scenarios where autonomous execution carries high financial/legal risk.
HIGH_RISK_SCENARIOS: Final[frozenset[Scenario]] = frozenset(
    {
...
class PatternExtractor:
    ...
    def __init__(
        ...
        *,
        ...
        self._log = logger.bind(component="pattern_extractor")
        ...
        *,
        ...
        if len(positive) < self._min_support:
            return ExtractionResult(
                ...
                skipped_reasons=(
                    f"insufficient_support: {len(positive)} < {self._min_support}",
                ),
            )
        ...
        defer_count = sum(
            1 for c in positive if c.decision.action_type is DecisionType.DEFER
        )
        ...
        for action_key, group_cases in grouped.items():
            ...
            min_support = (
                _MIN_SUPPORT_DEFER
                if action_key == DecisionType.DEFER.value
                else self._min_support
            )
            if len(group_cases) < min_support:
                skipped.append(
                    f"action_{action_key}_support_{len(group_cases)}"
                    f"_below_{min_support}",
                )
            ...
            if confidence < self._min_confidence:
                skipped.append(
                    f"action_{action_key}_confidence_{confidence:.2f}"
                    f"_below_{self._min_confidence:.2f}",
                )
                gate_decisions.append(
                    GateDecision(
                        action=action_key,
    ...
    def _infer_categorical_condition(
        ...
        neg_ratio = (
            neg_counter.get(most_common_value, 0) / len(neg_values)
            if neg_values
            else 0
        )
    ...
    def _infer_boolean_condition(
        ...
        if pos_true_ratio >= _DOMINANCE_THRESHOLD:
            if neg_values:
                neg_true_ratio = sum(1 for v in neg_values if v) / len(
                    neg_values
                )
    ...
    def _collect_snapshot_features(
        ...
        :data:`brain_engine.patterns.scenario_features.SCENARIO_FEATURES`
        is consulted via
        :func:`brain_engine.patterns.condition_synthesizer._resolve_feature_keys`
    ...
    def _compute_confidence(
        ...
        :mod:`brain_engine.patterns.wilson` for the reference formula.
...
def _summarise_refusal_signals(
    ...
    for case in cases:
        ...
        for signal in signals:
            ...
            if (
                refusal_type not in conditional_examples
...
def _count_signal_weights(
    ...
    :class:`brain_engine.patterns.confidence.ConfidenceContext` field
    ...
    * ``ResolutionType.PM_MODIFIED`` → ``pm_repeated_edit_count``.
    ...
    from brain_engine.patterns.models import ResolutionType
...
def _emit_extract_metrics(rules: list[PatternRule]) -> None:
    ...
    try:
        ...
        for rule in rules:
            exporter.record_pattern_rule_emitted(
                scenario=rule.scenario.value,
                conditional=bool(rule.conditions),
            )
    except Exception:
        return
