...
class PatternMiner:
    ...
    def __init__(
        ...
        *,
        ...
        self._log = logger.bind(component="pattern_miner")
        ...
        *,
        ...
        if self._forbidden_foundation_ids:
            cases = [
                case
                for case in cases
                if case.foundation_scenario_id
                not in self._forbidden_foundation_ids
            ]
        ...
        below_support = 0
        below_confidence = 0
        emitted_keys: set[tuple[str, Scenario, DecisionType]] = set()
        ...
        _emit_mine_metrics(rules=rules, attempts=attempts,
    ...
    def _bucket(
        ...
        *,
        ...
        dict[tuple[str, Scenario, DecisionType], list[DecisionCase]],
        int,
        int,
    ]:
        """Group cases by ``(scope_id, scenario, action_type)``."""
        buckets: dict[
            tuple[str, Scenario, DecisionType], list[DecisionCase]
        ] = defaultdict(list)
        considered = 0
        rejected = 0
        for case in cases:
    ...
    def _synthesise_minorities(
        ...
        *,
        ...
        emitted_keys: set[tuple[str, Scenario, DecisionType]],
        scope: PatternScope,
        ...
        for (scope_id, scenario), action_map in scenario_index.items():
            ...
            for action_type, target_cases in action_map.items():
                ...
                rules.append(rule)
                self._log.info(
                    "synthesised_conditional_rule",
                    scenario=scenario.value,
                    action=action_type.value,
                    scope_id=scope_id,
                    conditions=list(result.conditions.keys()),
    ...
    def _index_by_scenario(
        ...
        tuple[str, Scenario], dict[DecisionType, list[DecisionCase]]
    ]:
    ...
    def _build_conditional_rule(
        ...
        *,
        ...
        condition_total = (
            result.support_count + result.counterexample_count
        )
        ...
        if (
            _all_historical(target_cases)
            and confidence > self._historical_cap
        ):
    ...
    def _build_rule(
        ...
        *,
        ...
        if (
            _all_historical(cases)
            and effective_confidence > self._historical_cap
        ):
        ...
        if (
            ...
            invalidated.append(
                replace(
                    ...
                    deactivated_at=(
                        cand.deactivated_at
                        if cand.deactivated_at is not None
...
def _emit_mine_metrics(
    *,
    ...
    try:
        ...
        accepted = max(0, attempts - rejected)
        for _ in range(accepted):
            exporter.record_pattern_synthesis_attempt(
                scenario="aggregated", accepted=True,
            )
        for _ in range(rejected):
            exporter.record_pattern_synthesis_attempt(
                scenario="aggregated", accepted=False,
            )
    except Exception as exc:
        logger.warning(
            "pattern_miner_metric_emit_failed",
            error=str(exc),
