"""Google GenAI (Gemini) chat model provider.

Uses ``litellm`` as the transport layer to call Google models
(gemini-2.5-flash, gemini-2.5-pro). Supports tool calling and streaming.
"""
...
_LITELLM_PREFIX = "gemini/"


class GoogleChatModel(BaseChatModel):
    """Google GenAI-backed chat model via litellm.

    Args:
        model: Gemini model identifier (e.g. ``gemini-2.5-flash``).
        api_key: Google API key. Falls back to ``GEMINI_API_KEY`` env.
    ...
    async def _do_stream(
        ...
        """Stream tokens from Google GenAI via litellm.
