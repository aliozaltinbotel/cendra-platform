...
class TrustMeterService:
    ...
    def __init__(
        ...
        *,
        ...
        workflows: Iterable[WorkflowKind] = tuple(WorkflowKind),
    ) -> None:
        self._engine = engine
        self._gate = gate or PromotionGate()
        self._workflows: tuple[WorkflowKind, ...] = tuple(workflows)

    async def for_property(self, property_id: str) -> TrustMeterView:
        ...
        """
        stored = await self._engine.list_for_property(property_id)
        by_workflow = {r.workflow: r for r in stored}
        bands = tuple(
            self._band_for(property_id, kind, by_workflow.get(kind.value))
            for kind in self._workflows
        )
    ...
    def _band_for(
        ...
        if record is None:
            record = WorkflowAutonomy(
                property_id=property_id, workflow=kind.value,
            )
