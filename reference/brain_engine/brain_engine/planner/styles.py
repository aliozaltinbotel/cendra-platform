...
:class:`brain_engine.planner.selector.StyleSelector` (Moat #4) picks
one per decision.
...
class PlannerStyleSpec:
    ...
    description: str
    denylist: frozenset[CardActionKind] = frozenset()
    ...
    def forbids(self, kind: CardActionKind) -> bool:
    ...
    def cap_reversibility(
        ...
        ceiling_idx = _REVERSIBILITY_ORDER.index(
            self.reversibility_ceiling
        )
...
_COOPERATIVE = PlannerStyleSpec(
    ...
    description=(
        "Default style.  No structural denials; engine autonomy "
        "and reversibility win."
    ),
)
...
_VIP_WHITE_GLOVE = PlannerStyleSpec(
    ...
    description=(
        "High-touch hospitality.  No structural denials; "
        "preference weights favour review score over occupancy "
        "and ADR."
    ),
    preference_weights={
        "review_score": 0.6,
...
_AGGRESSIVE_REVENUE = PlannerStyleSpec(
    ...
    description=(
        "Revenue-first style.  No denials; full autonomy "
        "permitted; preference weights favour ADR and occupancy."
    ),
    preference_weights={
        "adr": 0.6,
        "occupancy": 0.3,
...
BUILTIN_STYLE_SPECS: Final[
    Mapping[PlannerStyleId, PlannerStyleSpec]
] = {
