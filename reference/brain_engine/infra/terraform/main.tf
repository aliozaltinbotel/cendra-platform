terraform {
  ...
  backend "s3" {
    bucket         = "brain-engine-terraform-state"
    key            = "infra/terraform.tfstate"
    region         = "us-east-1"
    encrypt        = true
    dynamodb_table = "brain-engine-tf-lock"
  }
}
...
# ─────────────────────────────────────────────
# ElastiCache Redis (Episodic Memory)
# ─────────────────────────────────────────────
...
resource "aws_elasticache_replication_group" "redis" {
  ...
  description          = "Redis for Brain Engine episodic memory"
...
# Route POST / (AG-UI endpoint) to backend
resource "aws_lb_listener_rule" "backend_agui" {
  listener_arn = aws_lb_listener.http.arn
  priority     = 20
...
resource "aws_ecs_task_definition" "backend" {
  ...
  container_definitions = jsonencode([{
    ...
    secrets = [
      ...
      { name = "ELEVENLABS_AGENT_ID", valueFrom = "${aws_secretsmanager_secret.app_secrets.arn}:ELEVENLABS_AGENT_ID::" },
      { name = "NUKI_API_KEY", valueFrom = "${aws_secretsmanager_secret.app_secrets.arn}:NUKI_API_KEY::" },
    ]
...
resource "aws_ecs_task_definition" "frontend" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "COPILOTKIT_BACKEND_URL", value = "http://${aws_lb.main.dns_name}" },
    ]
