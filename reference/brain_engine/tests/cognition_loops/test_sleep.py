...
def _decision(
    *,
    ...
    return ResolvedDecision(
        ...
        ace_cycle=ace,
        memory_op=op,
        evaluated_at=datetime(2026, 5, 10, tzinfo=timezone.utc),
    )


def test_empty_decisions_zero_counts() -> None:
    """No decisions → empty kind_counts + no playbook bump."""
    period = datetime(2026, 5, 10, tzinfo=timezone.utc)
...
def test_playbook_bump_when_threshold_met() -> None:
    ...
    period = datetime(2026, 5, 10, tzinfo=timezone.utc)
    decisions = [
        _decision(target=f"t{i}", applied_kind=MemoryOpKind.ADD)
        for i in range(MIN_DECISIONS_FOR_PLAYBOOK_BUMP)
    ]
    report = summarise_decisions(
        decisions,
        period_start=period,
...
def test_period_validation() -> None:
    ...
    with pytest.raises(ValueError, match="period_end"):
        summarise_decisions(
            (),
            period_start=datetime(2026, 5, 11, tzinfo=timezone.utc),
            period_end=datetime(2026, 5, 10, tzinfo=timezone.utc),
        )
