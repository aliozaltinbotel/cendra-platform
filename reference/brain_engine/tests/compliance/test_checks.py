...
def test_registration_id_skips_non_booking() -> None:
    ...
    assert (
        registration_id_required(
            _ctx(action_kind=CardActionKind.SEND_MESSAGE)
        )
        is None
    )
...
def test_hitl_required_review_severity() -> None:
    ...
    violation = hitl_required_for_high_risk(
        _ctx(action_kind=CardActionKind.ISSUE_REFUND)
    )
    assert violation is not None
...
def test_gdpr_art22_blocks_natural_person_decision() -> None:
    ...
    violation = gdpr_art22_consent(
        _ctx(is_natural_person_decision=True)
    )
    assert violation is not None
    assert violation.severity is ComplianceSeverity.BLOCK
...
def test_never_ai_action_blocks_protected_class() -> None:
    ...
    violation = never_ai_action(
        _ctx(
            extra={"never_ai_category": "screen_by_protected_class"}
        )
    )
    assert violation is not None
    assert violation.severity is ComplianceSeverity.BLOCK
...
def test_never_ai_action_skips_unknown_category() -> None:
    ...
    assert (
        never_ai_action(_ctx(extra={"never_ai_category": "x"}))
        is None
    )
