...
def test_missing_record_time_is_not_hidden() -> None:
    ...
    assert out is not None and out.content == "CUR"
...
def test_invalidated_at_or_after_returns_none() -> None:
    ...
    assert (
        reconstruct_as_of(_invalidated_node(), datetime(2026, 4, 1, tzinfo=UTC))
        is None
    )
...
def test_naive_datetime_treated_as_utc() -> None:
    ...
    assert out is not None and out.content == "V0"


# ── integration: get_entity_knowledge(as_of=...) over fakeredis ─────


async def test_get_entity_knowledge_as_of_wiring() -> None:
    ...
    current = await graph.get_entity_knowledge("guest-1")
    assert len(current) == 1
    assert current[0].content == "guest prefers late checkout"

    # as_of far in the future → still visible (recorded before then).
    future = await graph.get_entity_knowledge(
        "guest-1", as_of=datetime(2030, 1, 1, tzinfo=UTC),
    )
    assert len(future) == 1

    # as_of before it was recorded → not yet known → filtered out.
    past = await graph.get_entity_knowledge(
        "guest-1", as_of=datetime(2020, 1, 1, tzinfo=UTC),
    )
    assert past == []
