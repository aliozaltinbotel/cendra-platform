...
from brain_engine.patterns.router import PatternRuleRouter, _is_valid_at
...
def test_temporal_split_is_not_contradiction() -> None:
    ...
    assert _resolve_pattern_rule_contradictions(
        inform_rule, [defer_rule],
    ) == []
...
@pytest.mark.asyncio
async def test_router_match_default_anchor() -> None:
    rule = _rule(
        pattern_id="R",
        action=DecisionType.APPROVE,
        valid_from=_NOW - timedelta(days=30),
...
@pytest.mark.asyncio
async def test_router_match_point_in_time_filter() -> None:
    closed = _rule(
        pattern_id="OLD",
        action=DecisionType.APPROVE,
        valid_from=_NOW - timedelta(days=30),
