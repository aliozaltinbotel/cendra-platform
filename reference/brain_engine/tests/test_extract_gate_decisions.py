...
:class:`brain_engine.patterns.extractor.ExtractionResult` and a
...
def test_extraction_result_gate_decisions_defaults_empty() -> None:
    ...
    from brain_engine.patterns.extractor import ExtractionResult
...
@pytest.mark.asyncio
async def test_accepted_action_emits_gate_decision_with_confidence() -> None:
    ...
    inform_decisions = [
        g for g in result.gate_decisions if g.action == "inform"
    ]
...
@pytest.mark.asyncio
async def test_insufficient_support_emits_gate_decision_without_confidence(
) -> None:
    ...
    deny_decisions = [
        g for g in result.gate_decisions if g.action == "deny"
    ]
...
@pytest.mark.asyncio
async def test_low_confidence_emits_gate_decision_with_actual_score(
) -> None:
    ...
    cases = [
        *(_make_case(action=DecisionType.INFORM) for _ in range(5)),
        *(
            _make_case(action=DecisionType.INFORM, outcome=_negative_outcome())
            for _ in range(5)
        ),
    ]
    store = await _seed_store(cases)
    extractor = PatternExtractor(
        store=store, min_support=3, min_confidence=0.9,
    )
...
@pytest.mark.asyncio
async def test_defer_uses_lower_support_threshold() -> None:
    ...
    defer_decisions = [
        g for g in result.gate_decisions if g.action == "defer"
    ]
...
@pytest.mark.asyncio
async def test_skipped_reasons_remains_populated_alongside_decisions(
) -> None:
