...
| T4 | `api/core/moderation/cendra_brain/cendra_brain.py` (new module in registered dir) | 1 | Art. 50 disclosure + PII redaction moderation module (zero-edit Extensible registration; marker documents the T4 surface) | #5 | — |
| T5 | `api/extensions/ext_celery.py` | 1 | Additive beat_schedule entries — active only when brain beat tasks are enabled | #5 | — |
| T6 | `api/services/external_knowledge_service.py` (single marked block; serving side stays the zero-edit external-knowledge loopback **decided Batch 4**) | 1 | Brain memory served to knowledge nodes through Dify's External Knowledge Base API; the retrieval endpoint lands with the Batch 5 service layer at `api/controllers/service_api/brain/` (additive). One marked block in `fetch_external_knowledge_retrieval` threads the run's decision-time `as_of` into kernel-bound retrieve requests (CEN-15 §E1 / CEN-27) — kernel-endpoint-guarded, byte-identical request otherwise. Registered file is the request-building choke point covering both workflow-node and agent-tool retrieval paths (the `api/core/rag/retrieval/` path named in CLAUDE.md covers neither `dataset_retriever_tool` nor `datasource/retrieval_service`) | #4, #32 | — |
...
REGISTERED+='|.github/workflows/deploy-cendra-gcp.yml|deploy/|.gcloudignore|api/.gcloudignore)'   # keep in sync with T/C tables
